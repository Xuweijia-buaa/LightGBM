  ...
  void ConstructHistograms(const data_size_t* data_indices,
    ...
    if (cur_multi_val_bin != nullptr) {
      ...
      for (int block_id = 0; block_id < n_data_block_; ++block_id) {
        ...
        if (inner_hist_bits == 8) {
          ConstructHistogramsForBlock<USE_INDICES, ORDERED, USE_DIST_GRAD, 8>(
      ...
      if (inner_hist_bits == 8) {
        HistMerge<USE_DIST_GRAD, HIST_BITS, 8>(hist_buf);
      } else {
        HistMerge<USE_DIST_GRAD, HIST_BITS, HIST_BITS>(hist_buf);
      }
      global_timer.Stop("Dataset::sparse_bin_histogram_merge");
      global_timer.Start("Dataset::sparse_bin_histogram_move");
      if (inner_hist_bits == 8) {
        HistMove<USE_DIST_GRAD, HIST_BITS, 8>(*hist_buf);
      } else {
        HistMove<USE_DIST_GRAD, HIST_BITS, HIST_BITS>(*hist_buf);
      }
