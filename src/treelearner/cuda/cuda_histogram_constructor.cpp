...
void CUDAHistogramConstructor::CalcConstructHistogramKernelDim(
  ...
  *block_dim_y = NUM_THRADS_PER_BLOCK / cuda_row_data_->max_num_column_per_partition();
  const int num_data_per_thread = std::min(((65536 / (grad_discretized_bins_ + 2)) - 1) / (*block_dim_y), NUM_DATA_PER_THREAD);
  //Log::Warning("num_data_per_thread = %d, block_dim_y = %d", num_data_per_thread, *block_dim_y);
